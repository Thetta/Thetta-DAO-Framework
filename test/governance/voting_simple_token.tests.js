...
contract('Voting simple', (accounts) => {
	...
	describe('Voting()', function () {
		it('Check voting creation()',async() => {
			let voting = await Voting.new(daoBase.address, proposal.address, creator, VOTING_TYPE_SIMPLE_TOKEN, 0, 'Test', 100, 100, token.address);
		});
	});
	...
	describe('isFinished()', function () {
		...
		it('Should return true due to time elapsed',async() => {
			let voting = await Voting.new(daoBase.address, proposal.address, creator, VOTING_TYPE_SIMPLE_TOKEN, 60, 'Test', 100, 100, token.address);
	...
	describe('isYes()', function () {
		...
		it('Should return false due to quorum not reached',async() => {
			let voting = await Voting.new(daoBase.address, proposal.address, creator, VOTING_TYPE_SIMPLE_TOKEN, 0, 'Test', 70, 100, token.address);
			await voting.vote(true, {from: employee1});
			assert.equal(await voting.isYes(),false,'yes');
		});

		it('Should return false due to consensus not reached',async() => {
			let voting = await Voting.new(daoBase.address, proposal.address, creator, VOTING_TYPE_SIMPLE_TOKEN, 0, 'Test', 100, 70, token.address);
			await voting.vote(true, {from: employee1});
			assert.equal(await voting.isYes(),false,'yes');
		});
